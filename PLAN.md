...
- 各章の中心的な実装（つまり，章を跨いで利用されるような機械学習モデルの実装など）は `partX_hogehoge.py` にまとめ、可視化やデータ処理用の関数等の教育的価値を持たないものは`utils.py`に逃がす．そのうえで，`partX*.ipynb`に必要に応じて各章の`partX_hogehoge.py`と`utils.py`をインポートし，学習，可視化などを行う．その際に，大まかに何をやっているのかをマークダウンcellで記述するようにする．
...
- コードは短ければ短いほど良い．
- `pyproject.toml`は直接編集しない．importエラーが生じた場合に限って，`uv add`コマンドでライブラリを追加する事．
...
- pick-and-place タスクを定義する(env.py)。
  - 色付き立方体を配置する。
  - 目標位置をテーブル上に配置する。
  - 物体色、開始位置、目標位置を episode ごとにランダム化する。
- IK 専門家を実装する(part1_simulator.py)。
  - pre-grasp
  - grasp
  - lift
  - move-to-place
...
- 観測を保存する(part1_simulator.py)。
  ...
  - action qpos target
  - task text
- LeRobot Dataset 書き出しを実装する(part1_simulator.py)。
...
- 標準の `LeRobotDataset` 経由で画像、状態、アクションが取り出せる。
...
- LeRobot Dataset 用 DataLoader を用意する。
  - 標準の `LeRobotDataset`
  - 必要な章のNotebook内で action chunk を作る最小の collate 関数
